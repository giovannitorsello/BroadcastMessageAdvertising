...
const conferenceUri = `sip:${config.get("callflow.did")}@${config.get(
  "callflow.sbc"
)}`;
process.on("SIGINT", () => {
...
function runScenario(mediaserver) {
  ...
  logger.info(
    `starting ${rate} calls/sec, total ${total} limiting to ${limit} simultaneous calls`
  );

  intervalID = setInterval(
    checkCalls.bind(null, mediaserver, total, limit, rate),
    1000
  );
}
...
function checkCalls(mediaserver, total, limit, rate) {
  ...
  if (idx % 10 === 0) {
    logger.info(
      `${countSuccess}/${countFailure}/${currentCalls} - success/failure/in progress`
    );
  }

  if (countFinished >= total) {
    logger.info(
      `checkCalls: shutting down because total desired calls have been completed: ${countFinished}`
    );
    return do_shutdown();
  }
  ...
  if (currentCalls + countFinished + countStarting >= total) {
    if (!reachTotal || !(idx % 60)) {
      logger.info(
        `checkCalls: not starting any calls because we have reached our total: ${total}`
      );
    }
    reachTotal = true;
    return;
    ...
    if (!throttling) {
      logger.info(
        `checkCalls: not starting any calls because we have reached our limit: ${limit}`
      );
      throttling = true;
    }
    return;
  ...
  logger.debug(
    `total: ${total}, in progress: ${currentCalls}, finished: ${countFinished}, starting: ${countStarting}`
  );
  ...
  logger.debug(
    `checkCalls: starting ${countStart} calls with ${msInterval}ms delay`
  );
...
function do_shutdown() {
  ...
  if (callsInProgress.size)
    logger.info(
      `do_shutdown: killing ${callsInProgress.size} calls in progress`
    );
