...
*/
	...
	$goip_select="<select name=\"name\"  style=\"width:80px\" onchange=\"javascript:window.location='?name='+this.options[this.selectedIndex].value+'&sim_name=$_REQUEST[sim_name]&group_id=$_REQUEST[group_id]'\">\n\t<option value=\"0\" $bank_ch>All</option>\n";
	...
	$group_select="<select name=\"group_id\"  style=\"width:80px\" onchange=\"javascript:window.location='?name=$_REQUEST[name]&sim_name=$_REQUEST[sim_name]&group_id='+this.options[this.selectedIndex].value\">\n\t<option value=\"\" $group_ch>All</option>\n";
	$query=$db->query("SELECT * from sim_team ORDER BY sim_team_id");
	while($row=$db->fetch_array($query)) {
