...
if(isset($_GET['action'])) {
	...
	if($action=="recv"){
		...
		//if(_GET)
		$query=$db->query("(select sim.*,sim_team_name, 1 as 'in' from sim left join sim_team using (sim_team_id) where sim.sim_team_id=$id order by sim.id )
				union 
				(select *,NULL,0 from sim where sim_team_id=0 order by id ) 
				union
				(select sim.*,sim_team_name,2 from sim left join sim_team using (sim_team_id) where sim.sim_team_id!=0 and sim.sim_team_id!=$id order by id ) 
...
elseif($action=="receivers"){
	...
	WriteSuccessMsg("<br><li>修改成功</li>","sim_team.php?action=recv&id=$id");
                ...
                //if(_GET)
                $query=$db->query("(select device_line.*,sim_team_name, 1 as 'in' from device_line left join sim_team on sim_team.sim_team_id=device_line.goip_team_id where device_line.goip_team_id=$id order by line_name ASC)
                                union 
                                (select *,NULL,0 from device_line where goip_team_id=0 order by line_name ASC) 
                                union
                                (select device_line.*,sim_team_name,2 from device_line left join sim_team on sim_team.sim_team_id=device_line.goip_team_id where device_line.goip_team_id!=0 and device_line.goip_team_id!=$id order by line_name ASC)
                                        LIMIT $start_limit,$perpage");
